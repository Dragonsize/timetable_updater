...
def safe_request(
    ...
    """Execute HTTP request with strict redirect validation to trusted hosts."""
    current_url = url
...
class TDTUClient:
    ...
    def login(self) -> None:
        """Authenticate with TDTU student portal via AJAX SignIn endpoint."""
        try:
            # 1. Load login page to establish session cookies
            r1 = safe_request(self.session, "GET", PORTAL_LOGIN_URL, timeout=self.timeout)
            r1.raise_for_status()

            # 2. Submit credentials via AJAX POST
            payload = {
            ...
            try:
                ...
                raise TDTUAuthenticationError("SignIn response is not valid JSON") from exc
            ...
            if parsed_redirect.scheme != "https":
                raise TDTUAuthenticationError(f"Insecure redirect scheme: {parsed_redirect.scheme}")
            if (parsed_redirect.hostname or "") not in ALLOWED_HOSTS:
                raise TDTUAuthenticationError(f"Untrusted redirect domain: {parsed_redirect.hostname}")

            # 4. Follow redirect to land authenticated session
            ...
            if final_parsed.hostname not in ALLOWED_HOSTS:
                raise TDTUAuthenticationError(f"Final redirect on untrusted host: {final_parsed.hostname}")
            ...
            logger.info("TDTU login OK (Token=%s...)", self.token[:8] if self.token else "none")
