"""
ASP.NET WebForms state management and postback helper.
"""
...
class WebFormsPage:
    """Tracks state across ASP.NET WebForms postbacks."""
    ...
    def hidden_fields(self) -> dict[str, str]:
        """Extract hidden fields from current HTML."""
    ...
    def postback(
        ...
        """Execute ASP.NET postback with current WebForms state and update page state."""
        from tdtu.client import ALLOWED_HOSTS, safe_request, sanitize_url
        ...
        if parsed_target.scheme == "http" and (parsed_target.hostname or "") in ALLOWED_HOSTS:
            target_url = f"https://{parsed_target.netloc}{parsed_target.path}"
            if parsed_target.query:
                target_url += f"?{parsed_target.query}"
            parsed_target = urlparse(target_url)

        if parsed_target.scheme != "https":
            raise TDTUProtocolError(f"Insecure target URL for postback: {sanitize_url(target_url)}")
